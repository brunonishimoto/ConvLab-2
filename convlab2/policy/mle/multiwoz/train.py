...
class MLE_Trainer(MLE_Trainer_Abstract):
    def __init__(self, manager, cfg):
        ...
        vector = MultiWozVector(voc_file, voc_opp_file, composite_actions=cfg['composite_actions'], vocab_size=cfg['vocab_size'], domains=cfg['domains'])
        self._init_data(manager, cfg, vector)
        ...
        if cfg['composite_actions']:
            self.multi_entropy_loss = nn.CrossEntropyLoss()
            self.policy = DiscretePolicy(vector.state_dim, cfg['h_dim'], vector.da_dim).to(device=DEVICE)
        else:
...
if __name__ == '__main__':
    with open(os.path.join(root_dir, 'convlab2/policy/mle/multiwoz/config.json'), 'r') as f:
        cfg = json.load(f)

    manager = ActMLEPolicyDataLoaderMultiWoz(cfg)
