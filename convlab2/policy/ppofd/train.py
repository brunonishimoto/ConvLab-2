...
from convlab2.policy.rlmodule import ExpertMemory, ExpertTransition
...
def sampler(pid, queue, evt, env, policy, policy_expert, batchsz):
    """
    ...
    while sampled_num < batchsz:
        ...
        for t in range(traj_len):
            ...
            # save to queue
            buff.push(s_vec.numpy(), policy.vector.action_vectorize(a), r, next_s_vec.numpy(), mask, policy.vector.action_vectorize(a_e))
...
def sample(env, policy, policy_expert, batchsz, process_num):
    """
    ...
    for i in range(process_num):
        process_args = (i, queue, evt, env, policy, policy_expert, process_batchsz)
...
def update(env, policy, policy_expert, batchsz, epoch, process_num):
    # sample data asynchronously
    batch, metrics = sample(env, policy, policy_expert, batchsz, process_num)
    ...
    a_e = torch.Tensor(np.stack(batch.action_expert)).to(device=DEVICE)
    batchsz_real = s.size(0)

    policy.update(epoch, batchsz_real, s, a, r, mask, a_e)
...
if __name__ == '__main__':
    ...
    for exp in range(args.num_exp):
        ...
        policy_sys = PPOfD(True, domains=domains, exp=exp)
        policy_sys.load(args.load_path)

        # rule policy for warm up
        policy_expert = RulePolicy(character='sys', domains=domains)

        policy_expert.vector = policy_sys.vector
        ...
        for i in range(args.epoch):
            metrics = update(env, policy_sys, policy_expert, args.batchsz, i, args.process_num)
