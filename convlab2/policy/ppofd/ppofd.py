...
class PPOfD(Policy):
    ...
    def __init__(self, is_train=False, dataset='Multiwoz', domains=None, **kwargs):
        ...
        if dataset == 'Multiwoz':
            ...
            self.vector = MultiWozVector(voc_file, voc_opp_file, domains=domains)
        ...
        if is_train:
            ...
            self.imitation_loss = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    ...
    def update(self, epoch, batchsz, s, a, r, mask, a_exp):
        ...
        for i in range(self.update_round):
            ...
            v_target_shuf, A_sa_shuf, s_shuf, a_shuf, a_exp_shuf, log_pi_old_sa_shuf = v_target[perm], A_sa[perm], s[perm], a[perm], \
                                                                                       a_exp[perm], log_pi_old_sa[perm]

            # 2. get mini-batch for optimizing
            optim_chunk_num = int(np.ceil(batchsz / self.optim_batchsz))
            # chunk the optim_batch for total batch
            v_target_shuf, A_sa_shuf, s_shuf, a_shuf, a_exp_shuf, log_pi_old_sa_shuf = torch.chunk(v_target_shuf, optim_chunk_num), \
                                                                                       torch.chunk(A_sa_shuf, optim_chunk_num), \
                                                                                       torch.chunk(s_shuf, optim_chunk_num), \
                                                                                       torch.chunk(a_shuf, optim_chunk_num), \
                                                                                       torch.chunk(a_exp_shuf, optim_chunk_num), \
                                                                                       torch.chunk(log_pi_old_sa_shuf,
                                                                                                   optim_chunk_num)
            # 3. iterate all mini-batch to optimize
            policy_loss, value_loss = 0., 0.
            for v_target_b, A_sa_b, s_b, a_b, a_exp_b, log_pi_old_sa_b in zip(v_target_shuf, A_sa_shuf, s_shuf, a_shuf,
                                                                              a_exp_shuf, log_pi_old_sa_shuf):
            ...
            logging.info('<<dialog policy ppofd>> epoch {}, iteration {}, value, loss {}'.format(epoch, i, value_loss))
            logging.info('<<dialog policy ppofd>> epoch {}, iteration {}, policy, loss {}'.format(epoch, i, policy_loss))
    ...
    def save(self, directory, epoch):
        ...
        torch.save(self.value.state_dict(), directory + '/' + str(epoch) + '_ppofd.val.mdl')
        torch.save(self.policy.state_dict(), directory + '/' + str(epoch) + '_ppofd.pol.mdl')
    ...
    def load(self, filename):
        value_mdl_candidates = [
            filename + '.val.mdl',
            filename + '_ppofd.val.mdl',
            os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '.val.mdl'),
            os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '_ppofd.val.mdl')
        ]
        ...
        policy_mdl_candidates = [
            ...
            os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '_ppofd.pol.mdl')
        ]
    ...
    def load_from_pretrained(self, archive_file, model_file, filename):
        ...
        policy_mdl = os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '_ppofd.pol.mdl')
        if os.path.exists(policy_mdl):
            self.policy.load_state_dict(torch.load(policy_mdl, map_location=DEVICE))
            logging.info('<<dialog policy>> loaded checkpoint from file: {}'.format(policy_mdl))

        value_mdl = os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '_ppofd.val.mdl')
        if os.path.exists(value_mdl):
            self.value.load_state_dict(torch.load(value_mdl, map_location=DEVICE))
            logging.info('<<dialog policy>> loaded checkpoint from file: {}'.format(value_mdl))
