...
class RulePolicy(Policy):
    ...
    def __init__(self, is_train=False, character='sys', domains=None, generate_shared=False):
        self.is_train = is_train
        self.character = character

        if character == 'sys':
            self.policy = RuleBasedMultiwozBot(domains)
        elif character == 'usr':
            self.policy = UserPolicyAgendaMultiWoz(domains, generate_shared)
        else:
